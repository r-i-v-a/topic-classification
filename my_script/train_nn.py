...
in_vec = np.loadtxt('../../data/400_a_train.txt', usecols = (1:))

logging.info("Loading output vectors")
out_vec = np.loadtxt('../../data/400_a_train.txt', usecols = (0))
...
for kk in range(1,max_epochs):
    ...
    for jj, (X, Y) in enumerate(zip(np.array_split(in_vec, nsplits), 
        ...
        err, acc     = train(X.astype(np.float32), Y.astype(np.float32), lr)
        error       += err
        accuracy    += acc
        n           += len(X)
    ...
    for jj, (X, Y) in enumerate(zip(np.array_split(in_cv_vec, nsplits), np.array_split(out_cv_vec, nsplits))):
        err       = xentropy(X.astype(np.float32), Y.astype(np.float32))
        error    += err
        accuracy += acc
        n        += X.shape[0]
